...
async def _chat(client: httpx.AsyncClient, system: str, user: str) -> str:
    ...
    # Identify ourselves to the capture proxy, if one is in front of CHAT_URL.
    # These exchanges carry nothing an archive wants: the system prompt is
    # CLAUDE.md plus every Scope.md, the user prompt quotes a file already in
    # the vault, and the verdict is already persisted in filing_proposals.
    # Capturing them would duplicate the vault into itself on every cycle and
    # hand the filing agent a fresh candidate for every one it consumes.
